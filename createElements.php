...
function createTags(string $type, array $parameters = array(), bool $enclosed = false, string $content = ''): string
{
    $res = "<$type";
...
{
    return createTags('script', array(), true, $code);
}
function createScriptFromSrc(string $src): string
{
    return createTags('script', array('src' => $src), true);
}
